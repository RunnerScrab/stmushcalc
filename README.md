...
Installing it on my system (and hopefully yours) looks something like this:
```
...
`build.sh` is an ad-hoc build script I wrote for my convenience, but it may
work on your system. It needs the cargo tools in your path, which it assumes is
at `~/.cargo/bin`.
